<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 100 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 100이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

**Panel 100**은 사진의 주체, 동작, 관계를 소박한 실루엣, primitive forms, 소수의 민예 기호로 압축합니다. 크레용, 오일 파스텔, 굵은 연필의 입자를 남기고 사진에서 가장 생생한 색을 밝고 절제된 제한색으로 재구성합니다.

### 이런 경우에 적합합니다

- 사진의 주체·동작·관계를 소수 형태로 줄이면서 이야기를 지키고 싶을 때
- 소박한 실루엣, 평면 원근, 크레용·오일 파스텔·굵은 연필의 입자가 필요할 때
- 원본에서 뽑은 밝은 제한색, 민예 기호, 따뜻한 종이 여백을 살리고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 100로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 소박한 민예 서사 · primitive forms · flattened perspective · 크레용 입자 · 제한색 · 민예 편집 글씨
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 거친 실루엣, 손의 입자, 일상 서사가 함께 작동하는 민예 편집 포스터 |
| **대표 특징** | primitive forms, 평면 공간, 크레용 입자, 따뜻한 종이, 소수의 강조색, 민예 글씨 |
...
```text
주체와 관계 읽기 → 복잡한 배경 삭제 → 소박한 실루엣과 primitive forms로 압축 → flattened perspective로 이야기 재구성 → 원본의 생생한 소수 색 추출 → 건식 재료 입자와 민예 글씨로 완성
```

## 완성작의 특징

- 단순한 실루엣과 큰 primitive forms가 정체성과 이야기 관계를 유지합니다.
- 어두운 면은 균일하게 채우지 않고 문지름, 겹침, 끊김, 종이 비침을 남깁니다.
- 따뜻한 종이와 어두운 실루엣에 원본에서 나온 생생한 소수의 강조색을 더합니다.
- 글자는 간판, 메뉴, 메모, 출판 주석처럼 서사 안에 들어갑니다.
- 어린이 카툰, 귀여운 스티커, 사실주의, 매끈한 벡터, 복잡 원근, 3D, 템플릿을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-100-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 100의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-100 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-100 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-100 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-100 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-100 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-100 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 100 적용, 공통 설정은 한 번만 확정
/xxd-panel-100 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-100`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-100-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-100-prompt.zh-CN.md)
...
| **[xxd-panel-100](https://github.com/nevertoday/xxd-panel-100)** | 소박한 민예 서사 · primitive forms · 평면 공간 · 크레용 입자 · 따뜻한 종이 · 생생한 제한색 · 민예 편집 글씨 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–100) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
